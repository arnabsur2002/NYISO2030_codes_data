...
idl = [];														% load info according to load2disp.m
iimp = [];													% imports
iref = [];													% ref
for i = 1:length(mpcasein.genfuel)
  ...
  elseif strcmp('dfo', strtrim(mpcasein.genfuel{i}))
    idfo = [idfo; i];
  elseif strcmp('dl', strtrim(mpcasein.genfuel{i}))
    idl = [idl; i];
  elseif strcmp('import', strtrim(mpcasein.genfuel{i}))
    iimp = [iimp; i];   
  elseif strcmp('reference', strtrim(mpcasein.genfuel{i}))
    iref = [iref; i];   
  else
    ioth = [ioth; i];   
